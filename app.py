...
def generate_explanation(row, program):
    ...
    if group == 7:  # Foundation
        ...
        return "Eligible for Foundation"
    ...
    elif group == 2:  # CS + Certification
        if grade_to_numeric(row.get('MAT', 0)) >= 75:
            reasons.append(f"Math {row.get('MAT', '')} (≥B)")
        if grade_to_numeric(row.get('BI', 0)) >= 75:
            reasons.append(f"BI {row.get('BI', '')} (≥B)")
        if reasons:
            return "Eligible for CS + Certification: " + ", ".join(reasons)
        return "Eligible for CS + Certification"
    
    elif group == 3:  # CS Basic
    ...
    elif group == 4:  # English
        if grade_to_numeric(row.get('BI', 0)) >= 75:
            reasons.append(f"BI {row.get('BI', '')} (≥B)")
        return "Eligible for English: " + ", ".join(reasons) if reasons else "Eligible for English"
...
if search_button:
    with st.spinner("Searching for student..."):
        ...
        | **NOKP** | {row['NOKP']} |
        | **Name** | {row['NAMA']} |
        | **Gender** | {'Female' if row.get('JANTINA')=='P' else 'Male'} |
        | **Location** | {row.get('LOKASI', 'N/A')} |
        | **Stream** | {row.get('ALIRAN', 'N/A')} |
        | **Income** | RM {row.get('PENDAPATAN', 0):,.0f} |
                """)
            ...
            with col_right:
                ...
                all_programs_with_scores.sort(key=lambda x: -x['score'])
                
                # Show ALL programs
                st.caption(f"📊 Showing all {len(all_programs_with_scores)} programs")
                ...
                for i, prog in enumerate(all_programs_with_scores, 1):
                    if prog['eligible']:
                        ...
                        if grade_to_numeric(row.get('SEJ', 0)) < syarat.get('SEJ', 0):
                            reasons.append(f"Sejarah: {row.get('SEJ', 'N/A')}")
                        if grade_to_numeric(row.get('BM', 0)) < syarat.get('BM', 0):
                            reasons.append(f"BM: {row.get('BM', 'N/A')}")
                        if grade_to_numeric(row.get('MAT', 0)) < syarat.get('MAT', 0):
                            reasons.append(f"Math: {row.get('MAT', 'N/A')}")
                        ...
                        <div style='margin-bottom: 8px; padding: 8px; border-left: 5px solid #dc3545; border-radius: 5px; background-color: white; border: 1px solid #e0e0e0;'>
                            ...
                            <span style='font-size: 0.75em; color: #dc3545;'><b>❌ Not eligible:</b> {', '.join(reasons) if reasons else 'Minimum requirements not met'}</span>
                        </div>
                        """, unsafe_allow_html=True)
                
                # Original Choices Summary
                st.markdown("### 📋 Student's Original Choices")
                choice_rows = []
                for i, p in enumerate(original_choices, 1):
                    in_list = any(p.lower() in prog['name'].lower() for prog in all_programs_with_scores if prog['eligible'])
                    status = "✅ In Recommendations" if in_list else "❌ Not in Recommendations"
                    choice_rows.append([f"PIL{i}", p, status])
                
                if choice_rows:
                    import pandas as pd
                    df_choices = pd.DataFrame(choice_rows, columns=["Choice", "Program", "Status"])
