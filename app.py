...
if 'expanded_programs' not in st.session_state:
    st.session_state.expanded_programs = {}

# Display recommendations with custom expanders
for i, prog in enumerate(all_programs_with_scores, 1):
    if prog['eligible']:
        ...
        # Create a unique key for this program
        program_key = f"prog_{i}_{prog['name'][:20]}"
        ...
        # Create expander for detailed content
        with st.expander(f"📋 View Details - {prog['name'][:50]}", expanded=False):
