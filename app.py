...
def check_offered_program(program_ditawar, original_choices):
    if program_ditawar == 'TIDAK DITAWARKAN' or pd.isna(program_ditawar):
        return {'type': 'info', 'message': "❌ Student was not offered any program."}
    ...
    return {'type': 'info', 'message': f"✅ Program Offered: {program_ditawar}"}
    ...
    with st.spinner("Searching for student..."):
        ...
        if len(student) == 0:
            ...
            with col_right:
                ...
                # ========================================
                # PROGRAM RECOMMENDATIONS WITH SCORES IN HEADER
                # ========================================
                ...
                # Display recommendations with expanders - scores in header using st.markdown inside expander
                for i, prog in enumerate(all_programs_with_scores, 1):
                    if prog['eligible']:
                        ...
                        # Create expander with HTML header that shows name and scores together
                        # Using markdown inside expander to create the header
                        with st.expander(f"{i}. {star}{prog['name']}"):
                            # This is the header that appears INSIDE after click
                            # But we also want scores visible before click - Streamlit doesn't support custom HTML in expander title
                            # So we put scores right at the top inside
                            pass
                        
                        # Alternative: Use st.container with custom HTML for each card
                        # But since st.expander doesn't support custom header, we'll use a different approach
                        # Create a container with expander that has the scores at the top inside
                        with st.expander(f"{i}. {star}{prog['name']}"):
                            # Show scores at the very top of expanded content
                            st.markdown(f"""
                            <div style='display: flex; justify-content: flex-end; margin-bottom: 15px;'>
                                <div style='text-align: right;'>
                                    <span style='color: {score_color}; font-weight: bold; font-size: 1.1em;'>🎯 Total: {prog['total_score']}%</span><br>
                                    <span style='font-size: 0.8em; color: #666;'>📊 Eligibility: {prog['academic_score']}%</span>
                                </div>
                            </div>
                            """, unsafe_allow_html=True)
                            ...
                            <div style='margin: 10px 0; padding: 10px; background-color: #f8f9fa; border-radius: 8px;'>
                                <b>Formula:</b> {detailed['weight_formula']}<br>
                                <small>{detailed['formula_explanation']}</small>
                            </div>
                            """, unsafe_allow_html=True)
                            ...
                            with col_a:
                                st.markdown(f"**📚 Academic**\n{prog['academic_score']}%")
                                for subj in detailed['breakdown']['academic']['subjects']:
                                    st.markdown(f"- {subj['subject']} ({subj['grade']}): {subj['contribution']:.1f}")
                            
                            with col_b:
                                st.markdown(f"**🏠 Demographic**\n{prog['demographic_score']}%")
                                st.markdown(f"- Location: {row.get('LOKASI', 'N/A')}")
                                income_val = row.get('PENDAPATAN', 5000)
                                if income_val < 3000:
                                    inc_cat = "B40"
                                elif income_val < 5000:
                                    inc_cat = "M40 (Lower)"
                                elif income_val < 8000:
                                    inc_cat = "M40 (Upper)"
                                else:
                                    inc_cat = "T20"
                                st.markdown(f"- Income: RM {income_val:,.0f} ({inc_cat})")
                            
                            with col_c:
                                st.markdown(f"**⭐ Preference**\n{prog['preference_bonus']}/15")
                                if prog['in_original']:
