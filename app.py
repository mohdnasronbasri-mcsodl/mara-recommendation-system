...
with col2:
    st.image("https://photos.smugmug.com/REKABENTUK/LOGO/KOLEKSI-LOGO/i-j2SVDpd/0/Kkb8LcCXXZcpDqr9scB36rcD6FvXG2DM2gdpwB5kT/X2/logo%20mara%202021-01-X2.png", width=100)
st.sidebar.markdown("---")
    ...
    with st.spinner("Searching for student..."):
        ...
        if len(student) == 0:
            ...
            with col_left:
                ...
                # Format NOKP with leading zero (12 digits)
                nokp_display = str(row['NOKP']).zfill(12)
                
                st.markdown(f"""
                <div style='background-color: #FFFFFF; padding: 10px; border-radius: 8px; margin-bottom: 20px; border: 1px solid #e0e0e0;'>
                <table style='width:100%;'>
                     <tr><td style='padding: 6px;'><b>NOKP</b></td><td style='padding: 6px;'>{nokp_display}</td></tr>
